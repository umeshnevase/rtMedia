...
if ( ! class_exists( 'RTMediaAdmin' ) ) {
	...
	class RTMediaAdmin {
		...
		public function __construct() {
			...
			if ( filter_input( INPUT_POST, 'rtmedia-options' ) != NULL ) {
				$rtmedia_options = filter_input( INPUT_POST, 'rtmedia-options' );
				if ( isset( $rtmedia_options['general_showAdminMenu'] ) && $rtmedia_options['general_showAdminMenu'] == '1' ) {
		...
		function rtmedia_advertisement() {
			...
			<div class='rtmedia-admin-ad'>
				<a href='http://rtcamp.com/store/rtmedia-pro/' target='_blank' title='rtMedia Pro'>
					<img src='<?php wp_sprintf( $src ); ?>' alt="<?php _e( 'rtMedia Pro is released', 'rtmedia' ); ?>" />
				</a>
			</div>
			<?php
		}
		...
		function rtmedia_dashboard_widget_function() {
			echo '<div class="inside"><div class="table table_content"><p class="sub">' . __( 'Media Stats' ) . '</p><table><tbody>';
			...
			if ( $results ) {
				foreach ( $results as $media ) {
					if ( defined( strtoupper( 'RTMEDIA_' . $media->media_type . '_PLURAL_LABEL' ) ) ) {
						echo '<tr><td class = "b"> ' . $media->count . '</td><td class = "t">' . constant( strtoupper( 'RTMEDIA_' . $media->media_type . '_PLURAL_LABEL' ) ) . '</td></tr>';
					}
				}
			}
			echo '</tbody></table></div>'
			. '<div class="table table_discussion"><p class="sub">' . __( 'Usage Stats', 'rtmedia' ) . '</p><table><tbody> ';
			$sql					= "select count(*) from {$wpdb->users}";
			$results = $wpdb->get_var( $sql );
			echo '<tr>'
			. '<td class = "b">' . $results . '</td>'
			. '<td class = "t">' . __( 'Total ', 'rtmedia' ) . '</td>'
			. '</tr>';
			$sql	 			= "select count(distinct media_author) from {$rtMedia_model->table_name}";
			$results = $wpdb->get_var( $sql );
			echo '<tr><td class = "b">' . $results . '</td>'
			. '<td class = "t">' . __( 'With Media', 'rtmedia' ) . '</td></tr>';
			$sql	 			= "select count(*) from $wpdb->comments where comment_post_ID in (select media_id from {$rtMedia_model->table_name})";
			$results = $wpdb->get_var( $sql );
			echo '<tr><td class = "b">' . $results . '</td>'
			. '<td class = "t">' . __( 'Comments ', 'rtmedia' ) . '</td></tr>';
			$sql	 			= "select sum(likes) from {$rtMedia_model->table_name}";
			$results = $wpdb->get_var( $sql );
			echo '<tr><td class = "b">' . $results . '</td>'
			. '<td class = "t">' . __( 'Likes', 'rtmedia' ) . '</td></tr></tbody></table></div>'
			. '<div class="versions"><p>'
			. '<b>rtMedia Links:</b> <a href="http://rtcamp.com">' . __( 'Homepage', 'rtmedia' ) . '</a> '
			. '| <a href="admin.php?page=rtmedia-support#rtmedia-general">' . __( 'Free Support', 'rtmedia' ) . '</a> '
			. '| <a href="http://rtcamp.com/rtmedia/addons/">' . __( 'Premium Addons', 'rtmedia' ) . '</a>'
			. '</p></div></div>';
		}
		...
		public function ui( $hook ) {
			...
			if ( in_array( $hook, $admin_pages ) || strpos( $hook, 'rtmedia-migration' ) ) {
				...
				wp_localize_script(
					'rtmedia-admin', 'settings_url', add_query_arg(
						array( 'page' => 'rtmedia-settings' ), (is_multisite() ? network_admin_url( 'admin.php' ) : admin_url( 'admin.php' ) )
					) . '#privacy_enabled'
				);
				wp_localize_script(
					'rtmedia-admin', 'settings_rt_album_import_url', add_query_arg(
						array( 'page' => 'rtmedia-settings' ), (is_multisite() ? network_admin_url( 'admin.php' ) : admin_url( 'admin.php' ) )
					)
				);
		...
		function rt_regenerate_thumbnail() {
			...
			?>
			<div class="wrap">
				<h2> <?php _e( 'Regenerate Video Thumbnails', 'rtmedia' ); ?>
				</h2>
			<?php
			...
			echo '<p>' . __( 'Total Videos', 'rtmedia' ) . ' : <span class="rt-total">' .  $total . '</span></p>';
			echo '<p>' . __( 'Sent of regenerate thumbails', 'rtmedia' ) . ' : <span class="rt-done">0</span></p>';
			echo '<p>' . __( 'Fail to regenerate thumbails', 'rtmedia' ) . ' : <span class="rt-fail">0</span></p>'; ?>
			</div>
			<script>
			var db_done = 0;
					var db_fail = 0;
