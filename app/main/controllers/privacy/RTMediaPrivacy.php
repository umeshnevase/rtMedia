...
class RTMediaPrivacy {
    ...
    function __construct( $flag = true ) {
        if( is_rtmedia_privacy_enable() && $flag ) {
            ...
            add_filter( 'bp_activity_paged_activities_sql', array( $this, 'activity_privacy' ), 10, 2 );
    ...
    function add_nav() {
        ...
        $defaults = array( 'name' => $this->title(), // Display name for the nav item
    ...
    function activity_privacy( $sql, $r ) {
        if( is_rt_admin() ) {
            return $sql;
        }
        ...
        if( $activity_upgrade_done ) {
            ...
            $where_sql = 'WHERE a.date_recorded > \'' . $r[ 'filter' ][ 'since' ] . '\' AND a.is_spam = 0 AND a.hide_sitewide = 0 AND a.type NOT IN (\'activity_comment\', \'last_activity\')' . " AND (NOT EXISTS (SELECT m.activity_id FROM {$bp_prefix}bp_activity_meta m WHERE m.meta_key='rtmedia_privacy' AND m.activity_id=a.id) OR ( {$where} ) )";
            $sort = $r[ 'sort' ];
            $pag_sql = 'LIMIT 0, ' . $r[ 'per_page' ];
